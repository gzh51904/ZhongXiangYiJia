<template>
  <div class="desscase">
    ...
    <div class="fir" v-show="show">
      <div
        v-for="dess in desslist"
        :key="dess.productId"
        class="demsg"
        @click="gotogooditem(dess.productId,dess.skuId)"
      >
        <div class="dessleft">
        ...
        <div class="dessright">
          ...
          <div class="desssix">
            <span
              v-for="tag in (dess.tags)"
              :key="tag.tagName"
              :class="tag.tagName=='自营'?'tag':'tags'"
            >{{tag.tagName}}</span>
          </div>
        </div>
      </div>
...
.btn-back {
  ...
  height: 1.173333rem;
  background-repeat: no-repeat;
  background-image: url(../assets/img/icon-back-black.png);
...
.dessright {
  ...
  flex-direction: column;
  padding: 0.213333rem 0.4rem 0.213333rem 0.266667rem;
}
.desstitle {
...
.desssix .tag {
  margin: 0.053333rem 0.08rem 0 0;
  border: 0.026667rem solid #f51861;
  color: #f51861;
  font-size: 0.32rem;
...
.desssix .tags {
  margin: 0.133333rem 0.08rem 0 0;
  background: #f51861;
...
.itemmsg-title {
  ...
  word-break: break-all;
  font-size: 0.373333rem;
  padding: 0.08rem 0.4rem 0.08rem 0.133333rem;
  display: -webkit-box;
  -webkit-line-clamp: 2;
  overflow: hidden;
  height: 1.04rem;
